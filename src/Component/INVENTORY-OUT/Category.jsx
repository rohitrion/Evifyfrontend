...
const Category = ({ product, onProductSelect }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.status === 204) {
          ...
          setgetapi(response.data.distinct_values);
          console.log(response.data.distinct_values, "data fetched ")
          setFilteredData(response.data.distinct_values);
    ...
    <>
      <div>
        ...
        <div style={{ maxHeight: '800px', overflowX: 'hidden', scrollbarWidth: 'none', msOverflowStyle: 'none' }} >
          ...
          {
            <Category_data filteredData={filteredData} onProductSelect={onProductSelect}
